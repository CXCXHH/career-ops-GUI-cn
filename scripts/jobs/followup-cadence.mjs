...
function toDateStr(date) {
  // Format Date as YYYY-MM-DD in Asia/Shanghai timezone
  return date.toLocaleDateString('sv-SE', { timeZone: 'Asia/Shanghai' });
}

function today() {
  // Return today's date in Asia/Shanghai as a Date object at midnight
  const now = new Date();
  const shanghaiStr = now.toLocaleDateString('sv-SE', { timeZone: 'Asia/Shanghai' });
  return new Date(shanghaiStr);
}
...
function analyze() {
  ...
  if (apps.length === 0) {
    return {
      metadata: { analysisDate: toDateStr(new Date()), totalTracked: 0, actionable: 0, overdue: 0, urgent: 0, cold: 0, waiting: 0 },
      entries: [],
