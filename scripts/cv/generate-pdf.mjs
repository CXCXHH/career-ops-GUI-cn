...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: npm run pdf -- <input.html> <output.pdf> [--format=letter|a4]');
    process.exit(1);
  }
  ...
  const fontsDir = resolve(PROJECT_ROOT, 'fonts');
  html = html.replace(
    /url\(['"]?\.\/fonts\//g,
    `url('file://${fontsDir}/`
  );
  ...
  try {
    ...
    console.log(`⚠️  Playwright bundled Chromium unavailable; using system browser: ${fallback}`);
