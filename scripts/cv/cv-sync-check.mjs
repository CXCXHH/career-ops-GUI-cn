...
// 1. Check cv.md exists (optional in desensitized mode)
const cvPath = join(projectRoot, 'cv.md');
if (!existsSync(cvPath)) {
  warnings.push('cv.md not found in project root. This is optional in desensitized mode; create it when ready.');
} else {
...
// 2. Check profile.yml exists (optional in desensitized mode)
const profilePath = join(projectRoot, 'config', 'profile.yml');
if (!existsSync(profilePath)) {
  if (existsSync(join(projectRoot, 'config', 'profile.example.yml'))) {
    warnings.push('config/profile.yml not found. Copy from config/profile.example.yml and fill in your details when ready.');
  } else {
    warnings.push('config/profile.yml not found. Create it with your details when ready.');
  }
} else {
...
// 4. Check article-digest.md freshness (optional)
...
if (existsSync(digestPath)) {
  ...
  warnings.push('article-digest.md not found. This is optional; create it when you have proof points to track.');
}
