 ...
 *
 * Usage:
 *   npm run update:check              # Check if update available
 *   npm run update                    # Apply update (after user confirms)
 *   npm run rollback                  # Rollback last update
 *   node scripts/maintenance/update-system.mjs dismiss
 *
...
async function apply() {
  ...
  try {
    ...
    console.log(`Rollback available: npm run rollback`);
...
function dismiss() {
  ...
  console.log('Update check dismissed. Run "npm run update:check" or say "check for updates" to re-enable.');
}
...
switch (cmd) {
  ...
  default:
    console.log('Usage: node scripts/maintenance/update-system.mjs [check|apply|rollback|dismiss]');
    process.exit(1);
}
