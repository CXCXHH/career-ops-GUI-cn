...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  ...
  If yes → run `npm run update`. If no → run `node scripts/maintenance/update-system.mjs dismiss`.
...
- **GitHub Actions** run on every PR: `scripts/maintenance/test-all.mjs` (63+ checks), auto-labeler (risk-based: 🔴 core-architecture, ⚠️ agent-behavior, 📄 docs), welcome bot for first-time contributors
...
1. **NEVER edit applications.md to ADD new entries** -- Write TSV in `batch/tracker-additions/` and `npm run merge` handles the merge.
