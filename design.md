...
简历页面采用双板块设计，将「真实能力档案」和「岗位定制简历」分离：

### 板块 1：我的简历

用户的真实能力档案。AI 帮助提取整理，不编造任何内容。

- 支持上传 PDF、粘贴文本、手动填写三种方式
- 数据保存在 `resume-profile.json`
- 技能分组自动生成，展示更清晰

### 板块 2：岗位定制

针对特定岗位的简历生成。AI 基于真实档案进行定制。

- 选择目标岗位后，AI 自动匹配项目和技能
- 生成的简历可导出为 PDF
- 每次生成自动保存版本，可回溯对比

### 设计原则

- **AI 透明化：** AI 生成的项目会标记「AI 生成」标签
- **数据隔离：** 岗位定制不会修改原始档案
- **版本可追溯：** 所有定制简历都有版本记录
