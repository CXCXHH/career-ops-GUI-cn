...
- `{"status": "update-available", ...}` → tell the user an update is available and ask if they want to apply it (`npm run update`)
...
| `scripts/jobs/gemini-eval.mjs` | Standalone Gemini API evaluator (no CLI required) |
...
2. Run `npm run verify` to check health.
